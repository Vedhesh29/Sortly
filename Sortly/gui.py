...
class FileSorterGUI(QWidget):
    ...
    def __init__(self):
        ...
        if families:
            oswald_font = families[0]
            self.setFont(QFont(oswald_font, 10))
        else:
        ...
        # Sidebar for rules
        sidebar = QVBoxLayout()
        rule_label = QLabel("Custom Sorting Rules:")
        ...
        reset_btn.setToolTip("Restore original rules:\n" + "\n".join(
            [f"{ext} → {rule['folder']} ({rule['subfolder'] or 'no subfolder'})" for ext, rule in DEFAULT_RULES.items()]
        ))
    ...
    def show_context_menu(self, position):
        ...
        delete_action = menu.addAction("Delete Rule(s)")
        duplicate_action = menu.addAction("Duplicate Rule(s)")
        ...
        if action == delete_action:
            ...
            self.log("Deleted selected rule(s).")
            ...
            for row in sorted(selected_rows):
                ...
                if ext_item and folder_item:
                    ...
                    self.table.setItem(row_pos, 0, QTableWidgetItem(ext + "_copy"))
                    self.table.setItem(row_pos, 1, QTableWidgetItem(folder))
                    ...
                    self.table.setCellWidget(row_pos, 2, new_subfolder_combo)
            self.log("Duplicated selected rule(s).")
    ...
    def reset_to_default(self):
        ...
        self.log("Reset to default rules. All custom rules cleared.")
    ...
    def _sort_thread(self, folder, behavior):
        try:
            scan_and_sort(folder, self.sort_config, behavior)
